...
pub fn clear_package_cache(cache_base: &Path, full_name: Option<&str>) -> Result<(), String> {
    match full_name {
        Some(name) => {
            let mod_cache = cache_base.join(name);
            if mod_cache.exists() {
                std::fs::remove_dir_all(&mod_cache)
                    .map_err(|e| format!("Failed to clear cache for {}: {}", name, e))?;
            }
        }
        None => {
...
pub fn get_cache_path(cache_base: &Path, full_name: &str, version: &str) -> PathBuf {
    cache_base.join(full_name).join(version)
}

pub fn is_cached(cache_base: &Path, full_name: &str, version: &str) -> bool {
    let cache_dir = get_cache_path(cache_base, full_name, version);
    cache_dir.exists() && cache_dir.join(".cached").exists()
}
